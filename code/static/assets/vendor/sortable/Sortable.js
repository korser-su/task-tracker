...
(function (global, factory) {
    ...
    function _objectWithoutPropertiesLoose(source, excluded) {
        ...
        let target = {}, sourceKeys = Object.keys(source), key, i;
    ...
    function _objectWithoutProperties(source, excluded) {
        if (source == null) return {};
        let target = _objectWithoutPropertiesLoose(source, excluded), key, i;
        if (Object.getOwnPropertySymbols) {
    ...
    let captureMode = {capture: false, passive: false};
    ...
    function isScrolledPast(el, elSide, parentSide) {
        ...
        /* jshint boss:true */
        while (parent) {
            let parentSideVal = getRect(parent)[parentSide], visible = void 0;
    ...
    function index(el, selector) {
        ...
        while (el = el.previousElementSibling) {
            if (el.nodeName.toUpperCase() !== 'TEMPLATE' && el !== Sortable.clone && (!selector || matches(el, selector))) index++;
        }
        return index;
    }
    /**
    ...
    function throttle(callback, ms) {
        return function () {
            if (!_throttleTimeout) {
                ...
                _throttleTimeout = setTimeout(() => { _throttleTimeout = void 0; }, ms);
            }
        };
    }
    ...
    function clone(el) {
        let Polymer = window.Polymer, $ = window.jQuery || window.Zepto;
    ...
    let expando = 'Sortable' + new Date().getTime();
        ...
        return {
            captureAnimationState: function captureAnimationState() {
                ...
                children.forEach(function (child) {
                    ...
                    let fromRect = _objectSpread2({}, animationStates[animationStates.length - 1].rect); // If animating: compensate for current animation
            ...
            animateAll: function animateAll(callback) {
                ...
                animationStates.forEach(function (state) {
                    let time = 0, target = state.target, fromRect = target.fromRect, toRect = getRect(target),
            ...
            animate: function animate(target, currentRect, toRect, duration) {
                if (duration) {
                    ...
                    let elMatrix = matrix(this.el),
                        scaleX = elMatrix && elMatrix.a, scaleY = elMatrix && elMatrix.d,
                        translateX = (currentRect.left - toRect.left) / (scaleX || 1), translateY = (currentRect.top - toRect.top) / (scaleY || 1);
    ...
    let plugins = [], defaults = {initializeByDefault: true};
    let PluginManager = {
        mount: function mount(plugin) {
            // Set default static properties
            for (let option in defaults) {
                if (defaults.hasOwnProperty(option) && !(option in plugin)) plugin[option] = defaults[option];
            }
        ...
        pluginEvent: function pluginEvent(eventName, sortable, evt) {
            ...
            evt.cancel = () => { _this.eventCanceled = true; };
            let eventNameGlobal = eventName + 'Global';
            plugins.forEach(function (plugin) {
                ...
                if (sortable.options[plugin.pluginName] && sortable[plugin.pluginName][eventName]) sortable[plugin.pluginName][eventName](_objectSpread2({sortable: sortable}, evt));
            });
        },
    ...
    function dispatchEvent(_ref) {
        let sortable = _ref.sortable, rootEl = _ref.rootEl, name = _ref.name, targetEl = _ref.targetEl,
            cloneEl = _ref.cloneEl, toEl = _ref.toEl, fromEl = _ref.fromEl, oldIndex = _ref.oldIndex,
            newIndex = _ref.newIndex, oldDraggableIndex = _ref.oldDraggableIndex,
            newDraggableIndex = _ref.newDraggableIndex, originalEvent = _ref.originalEvent,
            putSortable = _ref.putSortable, extraEventProperties = _ref.extraEventProperties;
        sortable = sortable || rootEl && rootEl[expando];
        if (!sortable) return;
        let evt, options = sortable.options, onName = 'on' + name.charAt(0).toUpperCase() + name.substr(1); // Support for new CustomEvent feature
        if (window.CustomEvent && !IE11OrLess && !Edge) evt = new CustomEvent(name, {bubbles: true, cancelable: true});
        else {
        ...
        let allEventProperties = _objectSpread2(_objectSpread2({}, extraEventProperties), PluginManager.getEventProperties(name, sortable));
    ...
    let _excluded = ["evt"];
    let pluginEvent = function pluginEvent(eventName, sortable) {
        let _ref = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {}, originalEvent = _ref.evt, data = _objectWithoutProperties(_ref, _excluded);
    ...
    let dragEl, parentEl, ghostEl, rootEl, nextEl, lastDownEl, cloneEl, cloneHidden, oldIndex, newIndex,
        oldDraggableIndex, newDraggableIndex, activeGroup, putSortable, awaitingDragStarted = false,
        ignoreNextClick = false, sortables = [], tapEvt, touchEvt, lastDx, lastDy, tapDistanceLeft, tapDistanceTop,
        moved, lastTarget, lastDirection, pastFirstInvertThresh = false, isCircumstantialInvert = false,
        targetMoveDistance,
    ...
    /** @const */
    let documentExists = typeof document !== 'undefined',
        ...
        _detectNearestEmptySortable = function _detectNearestEmptySortable(x, y) {
            ...
            sortables.some(function (sortable) {
                let threshold = sortable[expando].options.emptyInsertThreshold;
        ...
        _prepareGroup = function _prepareGroup(options) {
            ...
            let group = {}, originalGroup = options.group;
    ...
    let nearestEmptyInsertDetectEvent = function nearestEmptyInsertDetectEvent(evt) {
        if (dragEl) {
            ...
            if (nearest) {
                ...
                let event = {};
                for (let i in evt) {
                    if (evt.hasOwnProperty(i)) event[i] = evt[i];
                }
    ...
    function Sortable(el, options) {
        ...
        for (let fn in this) {
            if (fn.charAt(0) === '_' && typeof this[fn] === 'function') this[fn] = this[fn].bind(this);
        } // Setup drag mode
        this.nativeDraggable = options.forceFallback ? false : supportDraggable;
        if (this.nativeDraggable) this.options.touchStartThreshold = 1; // Touch start threshold cannot be greater than the native dragstart threshold
        // Bind events
    ...
    Sortable.prototype = /** @lends Sortable.prototype */ {
        ...
        _onTapStart: function _onTapStart(evt) {
            ...
            let _this = this, el = this.el, options = this.options, preventOnFilter = options.preventOnFilter,
            ...
            if (lastDownEl === target) return; // Ignoring duplicate `down`
            // Get the index of the dragged element within its parent
            oldIndex = index(target);
            oldDraggableIndex = index(target, options.draggable); // Check filter
            if (typeof filter === 'function') {
                if (filter.call(this, evt, target, this)) {
                    _dispatchEvent({sortable: _this, rootEl: originalTarget, name: 'filter', targetEl: target, toEl: el, fromEl: el});
                ...
                filter = filter.split(',').some(function (criteria) {
                    ...
                    if (criteria) {
                        _dispatchEvent({sortable: _this, rootEl: criteria, name: 'filter', targetEl: target, fromEl: el, toEl: el});
                        pluginEvent('filter', _this, {evt: evt});
                        return true;
                    }
                });
        ...
        _prepareDragStart: function _prepareDragStart(evt, touch, target) {
            ...
            if (target && !dragEl && target.parentNode === el) {
                ...
                tapEvt = {target: dragEl, clientX: (touch || evt).clientX, clientY: (touch || evt).clientY};
                ...
                if (options.delay && (!options.delayOnTouchOnly || touch) && (!this.nativeDraggable || !(Edge || IE11OrLess))) {
                    ...
                    // before the delay has been reached: disable the delayed drag
        ...
        _onTouchMove: function _onTouchMove(evt) {
            if (tapEvt) {
                ...
                if (ghostEl) {
                    ...
                    } else ghostMatrix = {a: 1, b: 0, c: 0, d: 1, e: dx, f: dy};
        ...
        _onDragStart: function _onDragStart(evt, fallback) {
            let _this = this, dataTransfer = evt.dataTransfer, options = _this.options;
        ...
        _onDragOver: function _onDragOver(evt) {
            let el = this.el, target = evt.target, dragRect, targetRect, revert, options = this.options,
            ...
            function changed() {
                ...
                _dispatchEvent({sortable: _this, name: 'change', toEl: el, newIndex: newIndex, newDraggableIndex: newDraggableIndex, originalEvent: evt});
            }
            ...
            if (activeSortable && !options.disabled && (isOwner ? canSort || (revert = parentEl !== rootEl) // Reverting item into the original list
                ...
                if (!elLastChild || _ghostIsLast(evt, vertical, this) && !elLastChild.animated) {
                    ...
                    let nextSibling = target.nextElementSibling, after = direction === 1;
        ...
        _onDrop: function _onDrop(evt) {
            let el = this.el, options = this.options; // Get the index of the dragged element within its parent
            newIndex = index(dragEl);
            newDraggableIndex = index(dragEl, options.draggable);
            ...
            if (evt) {
                ...
                if (rootEl === parentEl || putSortable && putSortable.lastPutMode !== 'clone') cloneEl && cloneEl.parentNode && cloneEl.parentNode.removeChild(cloneEl); // Remove clone(s)
                if (dragEl) {
                    ...
                    _dispatchEvent({sortable: this, name: 'unchoose', toEl: parentEl, newIndex: null, newDraggableIndex: null, originalEvent: evt});
                    if (rootEl !== parentEl) {
                        if (newIndex >= 0) {
                            // Add event
                    ...
                    if (Sortable.active) {
                        ...
                        _dispatchEvent({sortable: this, name: 'end', toEl: parentEl, originalEvent: evt}); // Save sorting
                        this.save();
                    }
                }
            }
    ...
    function _onMove(fromEl, toEl, dragEl, dragRect, targetEl, targetRect, originalEvent, willInsertAfter) {
        let evt, sortable = fromEl[expando], onMoveFn = sortable.options.onMove, retVal; // Support for new CustomEvent feature
        if (window.CustomEvent && !IE11OrLess && !Edge) evt = new CustomEvent('move', {bubbles: true, cancelable: true});
        else {
            evt = document.createEvent('Event');
    ...
    Sortable.mount = function () {
        for (let _len = arguments.length, plugins = new Array(_len), _key = 0; _key < _len; _key++) {
    ...
    let autoScrolls = [], scrollEl, scrollRootEl, scrolling = false, lastAutoScrollX, lastAutoScrollY, touchEvt$1, pointerElemChangedInterval;
    function AutoScrollPlugin() {
        function AutoScroll() {
            this.defaults = {scroll: true, forceAutoScrollFallback: false, scrollSensitivity: 30, scrollSpeed: 10, bubbleScroll: true}; // Bind all private methods
            for (let fn in this) {
                if (fn.charAt(0) === '_' && typeof this[fn] === 'function') this[fn] = this[fn].bind(this);
            }
        }
        AutoScroll.prototype = {
            dragStarted: function dragStarted(_ref) {
                let originalEvent = _ref.originalEvent;
            ...
            dragOverCompleted: function dragOverCompleted(_ref2) {
                let originalEvent = _ref2.originalEvent;
            ...
            _handleAutoScroll: function _handleAutoScroll(evt, fallback) {
                let _this = this;
                let x = (evt.touches ? evt.touches[0] : evt).clientX, y = (evt.touches ? evt.touches[0] : evt).clientY,
    ...
    let autoScroll = throttle(function (evt, options, rootEl, isFallback) {
        // Bug: https://bugzilla.mozilla.org/show_bug.cgi?id=505521
        if (!options.scroll) return;
        let x = (evt.touches ? evt.touches[0] : evt).clientX, y = (evt.touches ? evt.touches[0] : evt).clientY,
            sens = options.scrollSensitivity, speed = options.scrollSpeed, winScroller = getWindowScrollingElement();
        let scrollThisInstance = false, scrollCustomFn; // New scroll root, set scrollEl
        ...
        let layersOut = 0, currentParent = scrollEl;
        do {
            let el = currentParent, rect = getRect(el),
                top = rect.top, bottom = rect.bottom,
                left = rect.left, right = rect.right,
                width = rect.width, height = rect.height,
                canScrollX = void 0, canScrollY = void 0,
                scrollWidth = el.scrollWidth, scrollHeight = el.scrollHeight,
                elCSS = css(el),
                scrollPosX = el.scrollLeft, scrollPosY = el.scrollTop;
            if (el === winScroller) {
            ...
            if (!autoScrolls[layersOut]) {
                for (let i = 0; i <= layersOut; i++) {
                    if (!autoScrolls[i]) autoScrolls[i] = {};
                }
            }
    ...
    Revert.prototype = {
        ...
        onSpill: function onSpill(_ref3) {
            let dragEl = _ref3.dragEl, putSortable = _ref3.putSortable;
    ...
    Remove.prototype = {
        onSpill: function onSpill(_ref4) {
            let dragEl = _ref4.dragEl, putSortable = _ref4.putSortable, parentSortable = putSortable || this.sortable;
    ...
    function SwapPlugin() {
        ...
        Swap.prototype = {
            ...
            dragOverValid: function dragOverValid(_ref2) {
                let completed = _ref2.completed, target = _ref2.target, onMove = _ref2.onMove, activeSortable = _ref2.activeSortable,
                    changed = _ref2.changed, cancel = _ref2.cancel;
            ...
            drop: function drop(_ref3) {
                let activeSortable = _ref3.activeSortable, putSortable = _ref3.putSortable, dragEl = _ref3.dragEl,
                    toSortable = putSortable || this.sortable, options = this.options;
    ...
    function MultiDragPlugin() {
        ...
        MultiDrag.prototype = {
            ...
            setupClone: function setupClone(_ref2) {
                let sortable = _ref2.sortable, cancel = _ref2.cancel;
            ...
            clone: function clone(_ref3) {
                let sortable = _ref3.sortable, rootEl = _ref3.rootEl, dispatchSortableEvent = _ref3.dispatchSortableEvent, cancel = _ref3.cancel;
                if (!this.isMultiDrag) return;
            ...
            showClone: function showClone(_ref4) {
                let cloneNowShown = _ref4.cloneNowShown, rootEl = _ref4.rootEl, cancel = _ref4.cancel;
                if (!this.isMultiDrag) return;
            ...
            hideClone: function hideClone(_ref5) {
                let _this = this, sortable = _ref5.sortable, cloneNowHidden = _ref5.cloneNowHidden, cancel = _ref5.cancel;
                if (!this.isMultiDrag) return;
                multiDragClones.forEach(function (clone) {
            ...
            dragStarted: function dragStarted(_ref7) {
                let _this2 = this;
                let sortable = _ref7.sortable;
                if (!this.isMultiDrag) return;
                if (this.options.sort) {
                    // Capture rects, hide multi drag elements (by positioning them absolute), set multi drag elements
                    // rects to dragRect, show multi drag elements, animate to rects, unset rects & remove from DOM
            ...
            dragOver: function dragOver(_ref8) {
                let target = _ref8.target, completed = _ref8.completed, cancel = _ref8.cancel;
            ...
            revert: function revert(_ref9) {
                let fromSortable = _ref9.fromSortable, rootEl = _ref9.rootEl, sortable = _ref9.sortable, dragRect = _ref9.dragRect;
            ...
            dragOverAnimationCapture: function dragOverAnimationCapture(_ref11) {
                let dragRect = _ref11.dragRect, isOwner = _ref11.isOwner, activeSortable = _ref11.activeSortable;
                multiDragElements.forEach(function (multiDragElement) {
            ...
            drop: function drop(_ref12) {
                let evt = _ref12.originalEvent, rootEl = _ref12.rootEl, parentEl = _ref12.parentEl,
                    sortable = _ref12.sortable, dispatchSortableEvent = _ref12.dispatchSortableEvent,
                    oldIndex = _ref12.oldIndex, putSortable = _ref12.putSortable, toSortable = putSortable || this.sortable;
                if (!evt) return;
                let options = this.options, children = parentEl.children; // Multi-drag selection
                if (!dragStarted) {
                    if (options.multiDragKey && !this.multiDragKeyDown) this._deselectMultiDrag();
                    toggleClass(dragEl$1, options.selectedClass, !~multiDragElements.indexOf(dragEl$1));
                    if (!~multiDragElements.indexOf(dragEl$1)) {
                        multiDragElements.push(dragEl$1);
                        dispatchEvent({sortable: sortable, rootEl: rootEl, name: 'select', targetEl: dragEl$1, originalEvent: evt}); // Modifier activated, select from last to dragEl
                        if (evt.shiftKey && lastMultiDragSelect && sortable.el.contains(lastMultiDragSelect)) {
                            let lastIndex = index(lastMultiDragSelect), currentIndex = index(dragEl$1);
                ...
                if (dragStarted && this.isMultiDrag) {
                    ...
                    if ((parentEl[expando].options.sort || parentEl !== rootEl) && multiDragElements.length > 1) {
                        let dragRect = getRect(dragEl$1), multiDragIndex = index(dragEl$1, ':not(.' + this.options.selectedClass + ')');
        ...
        return _extends(MultiDrag, {
            ...
            eventProperties: function eventProperties() {
                let _this3 = this, oldIndicies = [], newIndicies = [];
                multiDragElements.forEach(function (multiDragElement) {
                    oldIndicies.push({multiDragElement: multiDragElement, index: multiDragElement.sortableIndex}); // multiDragElements will already be sorted if folding
                    let newIndex;
